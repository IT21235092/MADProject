                        ...
                        .addValueEventListener(object : ValueEventListener {
                            override fun onDataChange(snapshot: DataSnapshot) {
                                ...
                                if (snapshot.exists()) {
                                    ...
                                    val mAdapter = EmpAdapter(payList)
                                    payRecyclerView.adapter = mAdapter

                                    mAdapter.setOnItemClickListener(object :
                                        EmpAdapter.onItemClickListener {
                                        override fun onItemClick(position: Int) {

                                            val intent = Intent(
                                                this@FetchingActivity,
                                                PaymentDetailsActivity::class.java
                                            )

                                            //put extras
                                            intent.putExtra("Payment ID", payList[position].empId)
                                            intent.putExtra("Payer", payList[position].payer)
                                            intent.putExtra("Job Type", payList[position].jobType)
                                            intent.putExtra(
                                                "Payment Per Hour",
